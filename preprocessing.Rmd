...
For performing only normalization on files and no debarcoding, you will have to provide an input file or files, bead masses and an output directory to store the output files and plots. Bead masses are just a list for mass channels corresponding to the beads separated by comma (,) .

```
cytoMine (--inputDir=<inputDir>  --outputDir=<results_dir> --mode=full --beadMasses=<beads>) [--sampleFile=<csvfile> --markerFile=<csvfile>]
...
```
cytoMine  --inputFiles=test.fcs --outputDir=results  --mode=full --beadMasses=140,151,153,165,175
--sampleFile=sample_info.csv --markerFile=markers.csv
```

--sampleFile and --markerFile are optional if you are using SampleFile name as Sample_info.csv and markerFile name as marker_list.csv
